<script setup lang="ts">
import { useI18n } from 'vue-i18n'

const { t } = useI18n()
</script>

<template>
  <div class="tw-mx-auto tw-max-w-4xl tw-py-4 xs:tw-py-16 tw-grid tw-grid-col-1 md:tw-grid-cols-6 tw-gap-4">
    <v-card
      variant="flat"
      class="tw-flex tw-flex-col tw-justify-between tw-p-4 tw-col-span-6 xs:tw-col-span-3 md:tw-col-span-2 md:tw-max-w-72 tw-rounded-xl border tw-border-gray-600 hover:tw-shadow-xl"
    >
      <div class="tw-flex tw-justify-center">
        <img src="@/assets/vue.svg" alt="Vue.js" />
      </div>
      <div class="tw-flex tw-flex-col tw-gap-2">
        <svg
          fill="currentColor"
          width="16"
          height="20"
          viewBox="0 0 24 24"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
        >
          <title>Vue icon</title>
          <path d="M24,1.61H14.06L12,5.16,9.94,1.61H0L12,22.39ZM12,14.08,5.16,2.23H9.59L12,6.41l2.41-4.18h4.43Z" />
        </svg>
        <p class="tw-text-lg tw-font-semibold">{{ t('skills.vue') }}</p>
        <p class="tw-text-sm tw-text-muted-foreground">
          <span class="tw-text-sm">{{ t('skills.vue_description') }}</span>
        </p>
      </div>
    </v-card>

    <v-card
      variant="flat"
      class="tw-flex tw-flex-col tw-justify-between tw-p-4 tw-col-span-6 xs:tw-col-span-3 md:tw-col-span-2 md:tw-max-w-72 tw-rounded-xl border tw-border-gray-600 hover:tw-shadow-xl tw-gap-2"
    >
      <div
        class="tw-overflow-hidden tw-w-full tw-h-full tw-min-h-24 tw-rounded-md tw-bg-gradient-to-r tw-from-indigo-500 tw-to-violet-500"
      ></div>
      <div class="tw-flex tw-flex-col tw-gap-2">
        <svg
          stroke="currentColor"
          fill="currentColor"
          stroke-width="0"
          role="img"
          viewBox="0 0 24 24"
          height="20"
          width="20"
          xmlns="http://www.w3.org/2000/svg"
        >
          <title>Tailwind CSS icon</title>
          <path
            d="M12.001,4.8c-3.2,0-5.2,1.6-6,4.8c1.2-1.6,2.6-2.2,4.2-1.8c0.913,0.228,1.565,0.89,2.288,1.624 C13.666,10.618,15.027,12,18.001,12c3.2,0,5.2-1.6,6-4.8c-1.2,1.6-2.6,2.2-4.2,1.8c-0.913-0.228-1.565-0.89-2.288-1.624 C16.337,6.182,14.976,4.8,12.001,4.8z M6.001,12c-3.2,0-5.2,1.6-6,4.8c1.2-1.6,2.6-2.2,4.2-1.8c0.913,0.228,1.565,0.89,2.288,1.624 c1.177,1.194,2.538,2.576,5.512,2.576c3.2,0,5.2-1.6,6-4.8c-1.2,1.6-2.6,2.2-4.2,1.8c-0.913-0.228-1.565-0.89-2.288-1.624 C10.337,13.382,8.976,12,6.001,12z"
          ></path>
        </svg>
        <p class="tw-text-lg tw-font-semibold">{{ t('skills.tailwind') }}</p>
        <p class="tw-text-sm tw-text-muted-foreground">
          <span class="tw-text-sm">{{ t('skills.tailwind_description') }}</span>
        </p>
      </div>
    </v-card>

    <v-card
      variant="flat"
      class="tw-flex tw-flex-col tw-justify-between tw-p-4 tw-col-span-6 xs:tw-col-span-3 md:tw-col-span-2 md:tw-max-w-72 tw-rounded-xl border tw-border-gray-600 hover:tw-shadow-xl"
    >
      <div class="tw-flex tw-justify-center">
        <svg
          fill="currentColor"
          width="76"
          height="96"
          viewBox="0 0 24 24"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
        >
          <title>TypeScript icon</title>
          <path
            d="M0 12v12h24V0H0zm19.341-.956c.61.152 1.074.423 1.501.865.221.236.549.666.575.77.008.03-1.036.73-1.668 1.123-.023.015-.115-.084-.217-.236-.31-.45-.633-.644-1.128-.678-.728-.05-1.196.331-1.192.967a.88.88 0 0 0 .102.45c.16.331.458.53 1.39.933 1.719.74 2.454 1.227 2.911 1.92.51.773.625 2.008.278 2.926-.38.998-1.325 1.676-2.655 1.9-.411.073-1.386.062-1.828-.018-.964-.172-1.878-.648-2.442-1.273-.221-.243-.652-.88-.625-.925.011-.016.11-.077.22-.141.108-.061.511-.294.892-.515l.69-.4.145.214c.202.308.643.731.91.872.766.404 1.817.347 2.335-.118a.883.883 0 0 0 .313-.72c0-.278-.035-.4-.18-.61-.186-.266-.567-.49-1.649-.96-1.238-.533-1.771-.864-2.259-1.39a3.165 3.165 0 0 1-.659-1.2c-.091-.339-.114-1.189-.042-1.531.255-1.197 1.158-2.03 2.461-2.278.423-.08 1.406-.05 1.821.053zm-5.634 1.002l.008.983H10.59v8.876H8.38v-8.876H5.258v-.964c0-.534.011-.98.026-.99.012-.016 1.913-.024 4.217-.02l4.195.012z"
          />
        </svg>
      </div>
      <div class="tw-flex tw-flex-col tw-gap-2">
        <svg
          fill="currentColor"
          width="16"
          height="20"
          viewBox="0 0 24 24"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
        >
          <title>TypeScript icon</title>
          <path
            d="M0 12v12h24V0H0zm19.341-.956c.61.152 1.074.423 1.501.865.221.236.549.666.575.77.008.03-1.036.73-1.668 1.123-.023.015-.115-.084-.217-.236-.31-.45-.633-.644-1.128-.678-.728-.05-1.196.331-1.192.967a.88.88 0 0 0 .102.45c.16.331.458.53 1.39.933 1.719.74 2.454 1.227 2.911 1.92.51.773.625 2.008.278 2.926-.38.998-1.325 1.676-2.655 1.9-.411.073-1.386.062-1.828-.018-.964-.172-1.878-.648-2.442-1.273-.221-.243-.652-.88-.625-.925.011-.016.11-.077.22-.141.108-.061.511-.294.892-.515l.69-.4.145.214c.202.308.643.731.91.872.766.404 1.817.347 2.335-.118a.883.883 0 0 0 .313-.72c0-.278-.035-.4-.18-.61-.186-.266-.567-.49-1.649-.96-1.238-.533-1.771-.864-2.259-1.39a3.165 3.165 0 0 1-.659-1.2c-.091-.339-.114-1.189-.042-1.531.255-1.197 1.158-2.03 2.461-2.278.423-.08 1.406-.05 1.821.053zm-5.634 1.002l.008.983H10.59v8.876H8.38v-8.876H5.258v-.964c0-.534.011-.98.026-.99.012-.016 1.913-.024 4.217-.02l4.195.012z"
          />
        </svg>
        <p class="tw-text-lg tw-font-semibold">{{ t('skills.typescript') }}</p>
        <p class="tw-text-sm tw-text-muted-foreground">
          <span class="tw-text-sm">{{ t('skills.typescript_description') }}</span>
        </p>
      </div>
    </v-card>

    <v-card
      variant="flat"
      class="tw-flex tw-flex-col tw-justify-between tw-p-4 tw-col-span-6 xs:tw-col-span-3 md:tw-col-span-2 md:tw-max-w-72 tw-rounded-xl border tw-border-gray-600 hover:tw-shadow-xl"
    >
      <div class="tw-flex tw-justify-center">
        <img src="@/assets/nodejs.svg" alt="Node.js" />
      </div>
      <div class="tw-flex tw-flex-col tw-gap-2">
        <svg
          fill="currentColor"
          width="16"
          height="20"
          viewBox="0 0 256 282"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
        >
          <title>Node icon</title>
          <g fill="currentColor">
            <path
              d="M116.504 3.58c6.962-3.985 16.03-4.003 22.986 0 34.995 19.774 70.001 39.517 104.99 59.303 6.581 3.707 10.983 11.031 10.916 18.614v118.968c.049 7.897-4.788 15.396-11.731 19.019-34.88 19.665-69.742 39.354-104.616 59.019-7.106 4.063-16.356 3.75-23.24-.646-10.457-6.062-20.932-12.094-31.39-18.15-2.137-1.274-4.546-2.288-6.055-4.36 1.334-1.798 3.719-2.022 5.657-2.807 4.365-1.388 8.374-3.616 12.384-5.778 1.014-.694 2.252-.428 3.224.193 8.942 5.127 17.805 10.403 26.777 15.481 1.914 1.105 3.852-.362 5.488-1.274 34.228-19.345 68.498-38.617 102.72-57.968 1.268-.61 1.969-1.956 1.866-3.345.024-39.245.006-78.497.012-117.742.145-1.576-.767-3.025-2.192-3.67-34.759-19.575-69.5-39.18-104.253-58.76a3.621 3.621 0 0 0-4.094-.006C91.2 39.257 56.465 58.88 21.712 78.454c-1.42.646-2.373 2.071-2.204 3.653.006 39.245 0 78.497 0 117.748a3.329 3.329 0 0 0 1.89 3.303c9.274 5.259 18.56 10.481 27.84 15.722 5.228 2.814 11.647 4.486 17.407 2.33 5.083-1.823 8.646-7.01 8.549-12.407.048-39.016-.024-78.038.036-117.048-.127-1.732 1.516-3.163 3.2-3 4.456-.03 8.918-.06 13.374.012 1.86-.042 3.14 1.823 2.91 3.568-.018 39.263.048 78.527-.03 117.79.012 10.464-4.287 21.85-13.966 26.97-11.924 6.177-26.662 4.867-38.442-1.056-10.198-5.09-19.93-11.097-29.947-16.55C5.368 215.886.555 208.357.604 200.466V81.497c-.073-7.74 4.504-15.197 11.29-18.85C46.768 42.966 81.636 23.27 116.504 3.58z"
            />
            <path
              d="M146.928 85.99c15.21-.979 31.493-.58 45.18 6.913 10.597 5.742 16.472 17.793 16.659 29.566-.296 1.588-1.956 2.464-3.472 2.355-4.413-.006-8.827.06-13.24-.03-1.872.072-2.96-1.654-3.195-3.309-1.268-5.633-4.34-11.212-9.642-13.929-8.139-4.075-17.576-3.87-26.451-3.785-6.479.344-13.446.905-18.935 4.715-4.214 2.886-5.494 8.712-3.99 13.404 1.418 3.369 5.307 4.456 8.489 5.458 18.33 4.794 37.754 4.317 55.734 10.626 7.444 2.572 14.726 7.572 17.274 15.366 3.333 10.446 1.872 22.932-5.56 31.318-6.027 6.901-14.805 10.657-23.56 12.697-11.647 2.597-23.734 2.663-35.562 1.51-11.122-1.268-22.696-4.19-31.282-11.768-7.342-6.375-10.928-16.308-10.572-25.895.085-1.619 1.697-2.748 3.248-2.615 4.444-.036 8.888-.048 13.332.006 1.775-.127 3.091 1.407 3.182 3.08.82 5.367 2.837 11 7.517 14.182 9.032 5.827 20.365 5.428 30.707 5.591 8.568-.38 18.186-.495 25.178-6.158 3.689-3.23 4.782-8.634 3.785-13.283-1.08-3.925-5.186-5.754-8.712-6.95-18.095-5.724-37.736-3.647-55.656-10.12-7.275-2.571-14.31-7.432-17.105-14.906-3.9-10.578-2.113-23.662 6.098-31.765 8.006-8.06 19.563-11.164 30.551-12.275z"
            />
          </g>
        </svg>

        <p class="tw-text-lg tw-font-semibold">{{ t('skills.node') }}</p>
        <p class="tw-text-sm tw-text-muted-foreground">
          <span class="tw-text-sm">{{ t('skills.node_description') }}</span>
        </p>
      </div>
    </v-card>

    <v-card
      variant="flat"
      class="tw-flex tw-flex-col tw-justify-between tw-p-4 tw-col-span-6 md:tw-col-span-4 tw-rounded-xl border tw-border-gray-600 hover:tw-shadow-xl"
    >
      <div class="tw-flex tw-justify-evenly">
        <div class="tw-flex tw-flex-col tw-items-center">
          <img src="@/assets/java.svg" alt="java" />
        </div>

        <div class="tw-flex tw-flex-col tw-items-center">
          <img src="@/assets/php.svg" alt="php" />
        </div>

        <div class="tw-flex tw-flex-col tw-items-center">
          <img src="@/assets/python.svg" alt="python" />
          <span class="tw-absolute tw-top-28">{{ t('skills.python') }}</span>
        </div>
      </div>
      <div class="tw-flex tw-flex-col tw-gap-2">
        <svg
          fill="currentColor"
          width="16"
          height="20"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 512 512"
          version="1.0"
        >
          <g transform="translate(0.000000,512.000000) scale(0.100000,-0.100000)" stroke="none">
            <path
              d="M2235 5106 c-74 -18 -169 -73 -219 -126 -73 -78 -103 -149 -132 -317 -14 -82 -29 -154 -33 -162 -4 -8 -46 -30 -93 -49 l-86 -35 -103 74 c-142 101 -148 105 -220 129 -79 27 -197 29 -269 5 -99 -33 -144 -68 -341 -264 -165 -165 -201 -206 -228 -262 -62 -127 -64 -275 -6 -390 14 -29 62 -104 107 -167 l81 -115 -38 -92 -37 -92 -81 -12 c-238 -36 -311 -64 -402 -156 -65 -65 -105 -137 -124 -223 -15 -70 -15 -534 0 -604 19 -86 59 -158 124 -223 91 -92 163 -120 400 -156 44 -6 85 -17 91 -23 6 -6 26 -48 44 -93 l33 -81 -82 -115 c-112 -159 -134 -209 -139 -323 -5 -102 10 -169 55 -250 34 -63 391 -421 456 -458 131 -75 295 -83 421 -20 27 14 101 61 163 106 l114 80 85 -32 c63 -25 86 -38 89 -54 3 -12 13 -73 24 -136 32 -193 67 -268 165 -357 57 -51 124 -85 204 -102 70 -15 534 -15 604 0 86 19 158 59 223 124 92 91 117 155 158 409 l12 80 92 37 91 37 116 -83 c125 -89 180 -119 248 -135 100 -23 224 -5 322 48 34 17 118 93 247 222 211 210 245 258 270 378 29 141 -7 258 -133 435 -41 56 -77 107 -80 112 -3 6 11 50 31 100 l38 90 79 12 c254 41 318 66 409 158 65 65 105 137 124 223 6 32 11 156 11 310 0 219 -3 267 -18 323 -25 89 -60 146 -128 208 -90 82 -142 102 -356 137 -60 9 -112 22 -117 28 -6 7 -25 48 -44 92 l-34 81 64 89 c134 187 163 254 163 380 0 83 -23 168 -64 234 -16 25 -115 132 -221 237 -161 161 -203 198 -256 223 -120 57 -247 64 -360 21 -31 -12 -114 -61 -183 -110 l-127 -89 -82 34 c-45 19 -89 38 -97 43 -9 5 -22 52 -36 135 -12 70 -30 151 -39 179 -47 147 -178 263 -333 296 -73 15 -551 13 -617 -3z m585 -302 c67 -27 78 -53 115 -264 20 -116 40 -203 52 -222 25 -43 69 -72 151 -99 37 -12 116 -44 176 -72 163 -75 137 -83 469 149 85 59 131 69 185 40 32 -17 355 -342 371 -373 16 -32 14 -89 -6 -126 -9 -18 -60 -94 -114 -170 -54 -77 -104 -156 -110 -178 -14 -46 -4 -90 42 -182 16 -32 45 -101 65 -155 38 -105 75 -155 128 -171 17 -5 105 -22 196 -36 183 -30 220 -43 253 -89 21 -30 22 -41 25 -275 5 -362 6 -361 -283 -406 -93 -15 -189 -35 -212 -45 -47 -21 -74 -57 -99 -135 -9 -27 -41 -105 -70 -171 -55 -123 -62 -161 -44 -209 6 -15 55 -89 110 -165 54 -76 106 -152 114 -169 19 -35 20 -78 5 -118 -12 -32 -331 -358 -371 -379 -35 -18 -92 -18 -129 2 -17 8 -93 60 -169 114 -76 55 -150 104 -165 110 -48 18 -85 11 -208 -44 -67 -29 -140 -59 -162 -66 -65 -20 -115 -52 -136 -87 -14 -22 -30 -90 -49 -210 -50 -305 -47 -302 -387 -303 -347 0 -339 -6 -389 287 -22 129 -40 205 -53 227 -21 34 -71 66 -136 86 -22 7 -92 35 -155 63 -119 52 -175 61 -220 37 -12 -6 -87 -58 -166 -115 -240 -171 -229 -173 -462 58 -96 95 -180 184 -188 199 -8 15 -14 47 -14 72 0 48 12 70 160 272 103 142 106 166 35 319 -20 44 -52 120 -70 169 -54 143 -67 150 -325 191 -280 45 -280 45 -280 385 0 342 -6 333 293 386 127 23 200 40 217 53 37 27 66 74 91 149 12 37 42 110 66 163 32 72 43 109 43 146 0 48 -6 58 -108 204 -123 174 -142 207 -142 244 0 52 32 93 208 268 146 145 184 177 208 178 78 3 91 -3 250 -118 82 -59 166 -113 186 -120 48 -16 83 -9 188 40 47 21 125 54 173 72 95 36 120 53 144 98 8 16 30 114 48 217 19 110 41 202 52 221 35 61 69 69 330 69 191 0 240 -3 273 -16z"
            />
            <path
              d="M2465 3549 c-231 -20 -468 -136 -630 -310 -357 -382 -357 -976 0 -1358 390 -417 1060 -417 1450 0 357 382 357 976 0 1358 -157 168 -401 291 -609 307 -39 3 -82 6 -96 8 -14 1 -65 -1 -115 -5z m273 -320 c306 -81 513 -351 513 -669 0 -388 -303 -691 -691 -691 -388 0 -691 303 -691 691 0 458 427 786 869 669z"
            />
          </g>
        </svg>

        <p class="tw-text-lg tw-font-semibold">{{ t('skills.backend') }}</p>
        <p class="tw-text-sm tw-text-muted-foreground">
          <span class="tw-text-sm">{{ t('skills.backend_description') }}</span>
        </p>
      </div>
    </v-card>

    <v-card
      variant="flat"
      class="tw-flex tw-flex-col tw-justify-between tw-p-4 tw-col-span-6 md:tw-col-span-3 tw-rounded-xl border tw-border-gray-600 hover:tw-shadow-xl"
    >
      <div class="tw-flex tw-justify-evenly">
        <div class="tw-flex tw-flex-col tw-items-center">
          <svg
            fill="currentColor"
            width="76"
            height="96"
            viewBox="0 0 24 24"
            role="img"
            xmlns="http://www.w3.org/2000/svg"
          >
            <title>PostgreSQL icon</title>
            <path
              d="M17.128 0a10.134 10.134 0 0 0-2.755.403l-.063.02A10.922 10.922 0 0 0 12.6.258C11.422.238 10.41.524 9.594 1 8.79.721 7.122.24 5.364.336 4.14.403 2.804.775 1.814 1.82.827 2.865.305 4.482.415 6.682c.03.607.203 1.597.49 2.879s.69 2.783 1.193 4.152c.503 1.37 1.054 2.6 1.915 3.436.43.419 1.022.771 1.72.742.49-.02.933-.235 1.315-.552.186.245.385.352.566.451.228.125.45.21.68.266.413.103 1.12.241 1.948.1.282-.047.579-.139.875-.27.011.33.024.653.037.98.041 1.036.067 1.993.378 2.832.05.137.187.843.727 1.466.54.624 1.598 1.013 2.803.755.85-.182 1.931-.51 2.649-1.532.71-1.01 1.03-2.459 1.093-4.809.016-.127.035-.235.055-.336l.169.015h.02c.907.041 1.891-.088 2.713-.47.728-.337 1.279-.678 1.68-1.283.1-.15.21-.331.24-.643s-.149-.8-.446-1.025c-.595-.452-.969-.28-1.37-.197a6.27 6.27 0 0 1-1.202.146c1.156-1.947 1.985-4.015 2.458-5.845.28-1.08.437-2.076.45-2.947.013-.871-.058-1.642-.58-2.309C21.36.6 19.067.024 17.293.004c-.055-.001-.11-.002-.165-.001zm-.047.64c1.678-.016 3.822.455 5.361 2.422.346.442.449 1.088.437 1.884-.013.795-.16 1.747-.429 2.79-.522 2.02-1.508 4.375-2.897 6.488a.756.756 0 0 0 .158.086c.29.12.951.223 2.27-.048.332-.07.575-.117.827.075a.52.52 0 0 1 .183.425.704.704 0 0 1-.13.336c-.255.383-.758.746-1.403 1.045-.571.266-1.39.405-2.116.413-.364.004-.7-.024-.985-.113l-.018-.007c-.11 1.06-.363 3.153-.528 4.108-.132.77-.363 1.382-.804 1.84-.44.458-1.063.734-1.901.914-1.038.223-1.795-.017-2.283-.428-.487-.41-.71-.954-.844-1.287-.092-.23-.14-.528-.186-.926-.046-.398-.08-.885-.103-1.434a51.426 51.426 0 0 1-.03-2.523 3.061 3.061 0 0 1-1.552.76c-.689.117-1.304.002-1.671-.09a2.276 2.276 0 0 1-.52-.201c-.17-.091-.332-.194-.44-.397a.56.56 0 0 1-.057-.381.61.61 0 0 1 .218-.331c.198-.161.46-.251.855-.333.719-.148.97-.249 1.123-.37.13-.104.277-.314.537-.622a1.16 1.16 0 0 1-.003-.041 2.96 2.96 0 0 1-1.33-.358c-.15.158-.916.968-1.85 2.092-.393.47-.827.74-1.285.759-.458.02-.872-.211-1.224-.552-.703-.683-1.264-1.858-1.753-3.186-.488-1.328-.885-2.807-1.167-4.067-.283-1.26-.45-2.276-.474-2.766-.105-2.082.382-3.485 1.217-4.37.836-.885 1.982-1.22 3.099-1.284 2.005-.115 3.909.584 4.294.734.742-.504 1.698-.818 2.892-.798a7.39 7.39 0 0 1 1.681.218l.02-.009a6.854 6.854 0 0 1 .739-.214A9.626 9.626 0 0 1 17.08.642zm.152.67h-.146a8.74 8.74 0 0 0-1.704.192c1.246.552 2.187 1.402 2.85 2.25a8.44 8.44 0 0 1 1.132 1.92c.11.264.184.487.226.66.021.087.035.16.04.236.002.038.004.077-.012.144 0 .003-.005.01-.006.013.03.876-.187 1.47-.213 2.306-.02.606.135 1.318.173 2.095.036.73-.052 1.532-.526 2.319.04.048.076.096.114.144 1.254-1.975 2.158-4.16 2.64-6.023.258-1.003.395-1.912.407-2.632.01-.72-.124-1.242-.295-1.46-1.342-1.716-3.158-2.153-4.68-2.165zm-4.79.256c-1.182.003-2.03.36-2.673.895-.663.553-1.108 1.31-1.4 2.085-.347.92-.466 1.81-.513 2.414l.013-.008c.357-.2.826-.4 1.328-.516.502-.115 1.043-.151 1.533.039s.895.637 1.042 1.315c.704 3.257-.219 4.468-.559 5.382a9.61 9.61 0 0 0-.331 1.013c.043-.01.086-.022.129-.026.24-.02.428.06.54.108.342.142.577.44.704.78.033.089.057.185.071.284a.336.336 0 0 1 .02.127 55.14 55.14 0 0 0 .013 3.738c.023.538.057 1.012.1 1.386.043.373.104.657.143.753.128.32.315.739.653 1.024.338.284.823.474 1.709.284.768-.165 1.242-.394 1.559-.723.316-.329.505-.787.626-1.488.181-1.05.545-4.095.589-4.668-.02-.432.044-.764.182-1.017.142-.26.362-.419.552-.505.095-.043.184-.072.257-.093a5.956 5.956 0 0 0-.243-.325 4.456 4.456 0 0 1-.666-1.099 8.296 8.296 0 0 0-.257-.483c-.133-.24-.301-.54-.477-.877-.352-.675-.735-1.493-.934-2.29-.198-.796-.227-1.62.281-2.201.45-.516 1.24-.73 2.426-.61-.035-.105-.056-.192-.115-.332a7.817 7.817 0 0 0-1.041-1.764c-1.005-1.285-2.632-2.559-5.146-2.6h-.115zm-6.642.052c-.127 0-.254.004-.38.011-1.01.058-1.965.351-2.648 1.075-.684.724-1.134 1.911-1.036 3.876.019.372.181 1.414.459 2.652.277 1.238.67 2.695 1.142 3.982.473 1.287 1.046 2.407 1.59 2.937.274.265.512.372.728.363.217-.01.478-.135.797-.518a43.244 43.244 0 0 1 1.81-2.048 3.497 3.497 0 0 1-1.167-3.15c.103-.739.117-1.43.105-1.976-.012-.532-.05-.886-.05-1.107a.336.336 0 0 1 0-.019v-.005l-.001-.006v-.001a9.893 9.893 0 0 1 .592-3.376c.28-.744.697-1.5 1.322-2.112-.614-.202-1.704-.51-2.884-.568a7.603 7.603 0 0 0-.38-.01zM18.199 6.9c-.679.009-1.06.184-1.26.413-.283.325-.31.895-.134 1.597.175.703.537 1.489.877 2.142.17.327.335.621.468.86.134.24.232.41.292.555.055.134.116.252.178.362.263-.555.31-1.1.283-1.668-.035-.703-.198-1.422-.174-2.15.027-.851.195-1.405.21-2.063a5.793 5.793 0 0 0-.74-.048zm-8.234.115a2.82 2.82 0 0 0-.616.074 4.665 4.665 0 0 0-1.153.449 2.417 2.417 0 0 0-.349.228l-.022.02c.006.146.035.5.047 1.021.012.57-.002 1.297-.112 2.084-.239 1.71 1.002 3.126 2.46 3.128.085-.351.225-.707.365-1.082.406-1.094 1.205-1.892.532-5.006-.11-.51-.328-.716-.628-.832a1.474 1.474 0 0 0-.524-.084zm7.917.204h.05c.066.002.127.009.18.022.054.012.1.03.138.055a.164.164 0 0 1 .075.11l-.001.008h.001-.001a.24.24 0 0 1-.035.135.668.668 0 0 1-.11.15.677.677 0 0 1-.386.212.59.59 0 0 1-.41-.103.608.608 0 0 1-.13-.118.26.26 0 0 1-.063-.127.17.17 0 0 1 .042-.128.384.384 0 0 1 .117-.09c.096-.054.226-.094.373-.116.055-.008.109-.012.16-.013zm-7.82.168c.053 0 .109.005.166.013.153.021.289.062.393.122a.446.446 0 0 1 .133.106.223.223 0 0 1 .054.17.302.302 0 0 1-.075.154.649.649 0 0 1-.143.13.64.64 0 0 1-.448.113.728.728 0 0 1-.42-.228.71.71 0 0 1-.118-.164.28.28 0 0 1-.041-.177c.015-.108.104-.164.191-.195a.866.866 0 0 1 .307-.04zm9.06 7.343l-.003.001c-.147.053-.268.075-.37.12a.452.452 0 0 0-.239.214c-.063.115-.117.319-.101.666a.51.51 0 0 0 .148.07c.171.052.458.086.778.081.638-.007 1.423-.156 1.84-.35a3.95 3.95 0 0 0 .943-.615h-.001c-1.393.288-2.18.211-2.663.012a1.315 1.315 0 0 1-.332-.2zm-8.031.094h-.021c-.053.005-.13.023-.279.188-.348.39-.47.635-.757.864-.287.228-.66.35-1.405.503-.236.048-.371.101-.461.144.029.024.026.03.07.053.109.06.249.113.362.142.32.08.846.173 1.395.08.549-.094 1.12-.357 1.607-1.04.084-.118.093-.292.024-.479-.07-.187-.223-.348-.331-.393a.653.653 0 0 0-.204-.06z"
            />
          </svg>
          <span>{{ t('skills.postgresql') }}</span>
        </div>

        <div class="tw-flex tw-flex-col tw-items-center">
          <img src="@/assets/mongodb-icon.svg" alt="mongodb" />
          <span>{{ t('skills.mongodb') }}</span>
        </div>
      </div>
      <div class="tw-flex tw-flex-col tw-gap-2">
        <svg
          fill="currentColor"
          width="16"
          height="20"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 512 512"
        >
          <title>Base de données icon</title>
          <path
            d="M256,219.3c45.2,0,87-4.3,126.3-11.9c39.3-8.5,70-20.5,93-36.7v48.6c0,12.8-9.4,25.6-29,36.7 c-19.6,11.1-46.1,20.5-80.2,26.5c-34.1,6-70,10.2-110.1,10.2s-75.9-3.4-110.1-10.2S85.3,267.1,65.7,256s-29-23-29-36.7v-48.6 c23,16.2,53.8,28.2,93,36.7C169,215,210.8,219.3,256,219.3z M256,438.6c45.2,0,87-4.3,126.3-11.9c39.3-8.5,70-20.5,93-36.7v48.6 c0,12.8-9.4,25.6-29,36.7c-19.6,11.1-46.1,20.5-80.2,26.5c-34.1,6-70.8,10.2-110.1,10.2c-39.3,0-75.9-3.4-110.1-10.2 c-34.1-6.8-60.6-15.4-80.2-26.5s-29-23-29-36.7V390c23,16.2,53.8,28.2,93,36.7C169,434.3,210.8,438.6,256,438.6z M256,329.4 c45.2,0,87-4.3,126.3-11.9s70-20.5,93-36.7v48.6c0,12.8-9.4,25.6-29,36.7c-19.6,11.1-46.1,20.5-80.2,26.5 c-34.1,6.8-70,10.2-110.1,10.2s-75.9-3.4-110.1-10.2s-60.6-15.4-80.2-26.5s-29-23-29-36.7v-48.6c23,16.2,53.8,28.2,93,36.7 S210.8,329.4,256,329.4z M256,0c39.3,0,75.9,3.4,110.1,10.2c34.1,6.8,60.6,15.4,80.2,26.5c19.6,11.1,29,23,29,36.7v36.7 c0,12.8-9.4,25.6-29,36.7c-19.6,11.1-46.1,20.5-80.2,26.5c-34.1,6-70,10.2-110.1,10.2s-75.9-3.4-110.1-10.2s-60.6-15.4-80.2-26.5 c-19.6-11.1-29-23-29-36.7V73.4c0-12.8,9.4-25.6,29-36.7s46.1-20.5,80.2-26.5C180.1,3.4,216.7,0,256,0z"
          />
        </svg>
        <p class="tw-text-lg tw-font-semibold">{{ t('skills.databases') }}</p>
        <p class="tw-text-sm tw-text-muted-foreground">
          <span class="tw-text-sm">{{ t('skills.databases_description') }}</span>
        </p>
      </div>
    </v-card>

    <v-card
      variant="flat"
      class="tw-flex tw-flex-col tw-justify-between tw-p-4 tw-col-span-6 md:tw-col-span-3 tw-rounded-xl border tw-border-gray-600 hover:tw-shadow-xl"
    >
      <div class="tw-flex tw-justify-evenly">
        <div class="tw-flex tw-flex-col tw-items-center">
          <img src="@/assets/windows.svg" alt="windows" />
          <span class="tw-absolute tw-top-28">{{ t('skills.windows') }}</span>
        </div>

        <div class="tw-flex tw-flex-col tw-items-center">
          <svg
            fill="currentColor"
            xmlns="http://www.w3.org/2000/svg"
            version="1.0"
            viewBox="0 0 266 312"
            width="76"
            height="96"
          >
            <path
              d="M128.6640625 79.2793c0 1-1 1-1 1h-1c-1 0-1-1-2-2 0 0-1-1-1-2s0-1 1-1l2 1c1 1 2 2 2 3m-18-10c0-5-2-8-5-8 0 0 0 1-1 1v2h3c0 2 1 3 1 5h2m35-5c2 0 3 2 4 5h2c-1-1-1-2-1-3s0-2-1-3-2-2-3-2c0 0-1 1-2 1 0 1 1 1 1 2m-30 16c-1 0-1 0-1-1s0-2 1-3c2 0 3-1 3-1 1 0 1 1 1 1 0 1-1 2-3 4h-1m-11-1c-4-2-5-5-5-10 0-3 0-5 2-7 1-2 3-3 5-3s3 1 5 3c1 3 2 6 2 9v2h1v-1c1 0 1-2 1-6 0-3 0-6-2-9s-4-5-8-5c-3 0-6 2-7 5-2 4-2.4 7-2.4 12 0 4 1.4 8 5.4 12 1-1 2-1 3-2m125 141c1 0 1-.4 1-1.3 0-2.2-1-4.8-4-7.7-3-3-8-4.9-14-5.7-1-.1-2-.1-2-.1-1-.2-1-.2-2-.2-1-.1-3-.3-4-.5 3-9.3 4-17.5 4-24.7 0-10-2-17-6-23s-8-9-13-10c-1 1-1 1-1 2 5 2 10 6 13 12 3 7 4 13 4 20 0 5.6-1 13.9-5 24.5-4 1.6-8 5.3-11 11.1 0 .9 0 1.4 1 1.4 0 0 1-.9 2-2.6 2-1.7 3-3.4 5-5.1 3-1.7 5-2.6 8-2.6 5 0 10 .7 13 2.1 4 1.3 6 2.7 7 4.3 1 1.5 2 2.9 3 4.2 0 1.3 1 1.9 1 1.9m-92-145c-1-1-1-3-1-5 0-4 0-6 2-9 2-2 4-3 6-3 3 0 5 2 7 4 1 3 2 5 2 8 0 5-2 8-6 9 0 0 1 1 2 1 2 0 3 1 5 2 1-6 2-10 2-15 0-6-1-10-3-13-3-3-6-4-10-4-3 0-6 1-9 3-2 3-3 5-3 8 0 5 1 9 3 13 1 0 2 1 3 1m12 16c-13 9-23 13-31 13-7 0-14-3-20-8 1 2 2 4 3 5l6 6c4 4 9 6 14 6 7 0 15-4 25-11l9-6c2-2 4-4 4-7 0-1 0-2-1-2-1-2-6-5-16-8-9-4-16-6-20-6-3 0-8 2-15 6-6 4-10 8-10 12 0 0 1 1 2 3 6 5 12 8 18 8 8 0 18-4 31-14v2c1 0 1 1 1 1m23 202c4 7.52 11 11.3 19 11.3 2 0 4-.3 6-.9 2-.4 4-1.1 5-1.9 1-.7 2-1.4 3-2.2 2-.7 2-1.2 3-1.7l17-14.7c4-3.19 8-5.98 13-8.4 4-2.4 8-4 10-4.9 3-.8 5-2 7-3.6 1-1.5 2-3.4 2-5.8 0-2.9-2-5.1-4-6.7s-4-2.7-6-3.4-4-2.3-7-5c-2-2.6-4-6.2-5-10.9l-1-5.8c-1-2.7-1-4.7-2-5.8 0-.3 0-.4-1-.4s-3 .9-4 2.6c-2 1.7-4 3.6-6 5.6-1 2-4 3.8-6 5.5-3 1.7-6 2.6-8 2.6-8 0-12-2.2-15-6.5-2-3.2-3-6.9-4-11.1-2-1.7-3-2.6-5-2.6-5 0-7 5.2-7 15.7v31.1c0 .9-1 2.9-1 6-1 3.1-1 6.62-1 10.6l-2 11.1v.17m-145-5.29c9.3 1.36 20 4.27 32.1 8.71 12.1 4.4 19.5 6.7 22.2 6.7 7 0 12.8-3.1 17.6-9.09 1-1.94 1-4.22 1-6.84 0-9.45-5.7-21.4-17.1-35.9l-6.8-9.1c-1.4-1.9-3.1-4.8-5.3-8.7-2.1-3.9-4-6.9-5.5-9-1.3-2.3-3.4-4.6-6.1-6.9-2.6-2.3-5.6-3.8-8.9-4.6-4.2.8-7.1 2.2-8.5 4.1s-2.2 4-2.4 6.2c-.3 2.1-.9 3.5-1.9 4.2-1 .6-2.7 1.1-5 1.6-.5 0-1.4 0-2.7.1h-2.7c-5.3 0-8.9.6-10.8 1.6-2.5 2.9-3.8 6.2-3.8 9.7 0 1.6.4 4.3 1.2 8.1.8 3.7 1.2 6.7 1.2 8.8 0 4.1-1.2 8.2-3.7 12.3-2.5 4.3-3.8 7.5-3.8 9.78 1 3.88 7.6 6.61 19.7 8.21m33.3-90.9c0-6.9 1.8-14.5 5.5-23.5 3.6-9 7.2-15 10.7-19-.2-1-.7-1-1.5-1l-1-1c-2.9 3-6.4 10-10.6 20-4.2 9-6.4 17.3-6.4 23.4 0 4.5 1.1 8.4 3.1 11.8 2.2 3.3 7.5 8.1 15.9 14.2l10.6 6.9c11.3 9.8 17.3 16.6 17.3 20.6 0 2.1-1 4.2-4 6.5-2 2.4-4.7 3.6-7 3.6-.2 0-.3.2-.3.7 0 .1 1 2.1 3.1 6 4.2 5.7 13.2 8.5 25.2 8.5 22 0 39-9 52-27 0-5 0-8.1-1-9.4v-3.7c0-6.5 1-11.4 3-14.6s4-4.7 7-4.7c2 0 4 .7 6 2.2 1-7.7 1-14.4 1-20.4 0-9.1 0-16.6-2-23.6-1-6-3-11-5-15l-6-9c-2-3-3-6-5-9-1-4-2-7-2-12-3-5-5-10-8-15-2-5-4-10-6-14l-9 7c-10 7-18 10-25 10-6 0-11-1-14-5l-6-5c0 3-1 7-3 11l-6.3 12c-2.8 7-4.3 11-4.6 14-.4 2-.7 4-.9 4l-7.5 15c-8.1 15-12.2 28.9-12.2 40.4 0 2.3.2 4.7.6 7.1-4.5-3.1-6.7-7.4-6.7-13m71.6 94.6c-13 0-23 1.76-30 5.25v-.3c-5 6-10.6 9.1-18.4 9.1-4.9 0-12.6-1.9-23-5.7-10.5-3.6-19.8-6.36-27.9-8.18-.8-.23-2.6-.57-5.5-1.03-2.8-.45-5.4-.91-7.7-1.37-2.1-.45-4.5-1.13-7.1-2.05-2.5-.79-4.5-1.82-6-3.07-1.38-1.26-2.06-2.68-2.06-4.27 0-1.6.34-3.31 1.02-5.13.64-1.1 1.34-2.2 2.04-3.2.7-1.1 1.3-2.1 1.7-3.1.6-.9 1-1.8 1.4-2.8.4-.9.8-1.8 1-2.9.2-1 .4-2 .4-3s-.4-4-1.2-9.3c-.8-5.2-1.2-8.5-1.2-9.9 0-4.4 1-7.9 3.2-10.4s4.3-3.8 6.5-3.8h11.5c.9 0 2.3-.5 4.4-1.7.7-1.6 1.3-2.9 1.7-4.1.5-1.2.7-2.1.9-2.5.2-.6.4-1.2.6-1.7.4-.7.9-1.5 1.6-2.3-.8-1-1.2-2.3-1.2-3.9 0-1.1 0-2.1.2-2.7 0-3.6 1.7-8.7 5.3-15.4l3.5-6.3c2.9-5.4 5.1-9.4 6.7-13.4 1.7-4 3.5-10 5.5-18 1.6-7 5.4-14 11.4-21l7.5-9c5.2-6 8.6-11 10.5-15s2.9-9 2.9-13c0-2-.5-8-1.6-18-1-10-1.5-20-1.5-29 0-7 .6-12 1.9-17s3.6-10 7-14c3-4 7-8 13-10s13-3 21-3c3 0 6 0 9 1 3 0 7 1 12 3 4 2 8 4 11 7 4 3 7 8 10 13 2 6 4 12 5 20 1 5 1 10 2 17 0 6 1 10 1 13 1 3 1 7 2 12 1 4 2 8 4 11 2 4 4 8 7 12 3 5 7 10 11 16 9 10 16 21 20 32 5 10 8 23 8 36.9 0 6.9-1 13.6-3 20.1 2 0 3 .8 4 2.2s2 4.4 3 9.1l1 7.4c1 2.2 2 4.3 5 6.1 2 1.8 4 3.3 7 4.5 2 1 5 2.4 7 4.2 2 2 3 4.1 3 6.3 0 3.4-1 5.9-3 7.7-2 2-4 3.4-7 4.3-2 1-6 3-12 5.82-5 2.96-10 6.55-15 10.8l-10 8.51c-4 3.9-8 6.7-11 8.4-3 1.8-7 2.7-11 2.7l-7-.8c-8-2.1-13-6.1-16-12.2-16-1.94-29-2.9-37-2.9"
            />
          </svg>
          <span>{{ t('skills.linux') }}</span>
        </div>
      </div>
      <div class="tw-flex tw-flex-col tw-gap-2">
        <svg
          fill="currentColor"
          width="16"
          height="20"
          role="img"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
        >
          <title>système d'exploitation</title>
          <path
            d="M 12 1 C 5.925 1 1 5.925 1 12 C 1 18.075 5.925 23 12 23 C 18.075 23 23 18.075 23 12 C 23 5.925 18.075 1 12 1 z M 8.625 7.5 C 11.654 7.5 11.78125 10.55975 11.78125 11.34375 L 11.78125 12.78125 C 11.78125 13.39525 11.704 16.5 8.625 16.5 C 5.551 16.5 5.46875 13.49225 5.46875 12.78125 L 5.46875 11.34375 C 5.46875 9.90375 6.001 7.5 8.625 7.5 z M 15.78125 7.5 C 18.32825 7.5 18.53125 9.70075 18.53125 10.21875 L 16.78125 10.21875 C 16.78125 10.03475 16.75 8.9375 15.75 8.9375 C 14.813 8.9375 14.78125 9.82775 14.78125 9.96875 C 14.78225 11.55475 18.53125 10.9935 18.53125 14.0625 C 18.53125 16.3965 16.2295 16.5 15.8125 16.5 C 13.4515 16.5 12.78125 14.771 12.78125 13.625 L 14.5625 13.625 C 14.5625 13.89 14.4825 15.0625 15.8125 15.0625 C 16.7465 15.0625 16.75 14.23475 16.75 14.09375 C 16.75 12.19975 13 12.9415 13 9.9375 C 13 7.6475 15.37225 7.5 15.78125 7.5 z M 8.625 8.96875 C 8.396 8.96875 7.21875 8.8385 7.21875 11.3125 L 7.21875 12.78125 C 7.21875 13.17825 7.144 15.03125 8.625 15.03125 C 10.064 15.03125 10 13.17825 10 12.78125 L 10 11.3125 C 10 10.8055 10.092 8.96875 8.625 8.96875 z"
          />
        </svg>
        <p class="tw-text-lg tw-font-semibold">{{ t('skills.os') }}</p>
        <p class="tw-text-sm tw-text-muted-foreground">
          <span class="tw-text-sm">{{ t('skills.os_description') }}</span>
        </p>
      </div>
    </v-card>
  </div>
</template>

<style scoped></style>
